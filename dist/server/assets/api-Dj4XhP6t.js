import { S as Subscribable, J as pendingThenable, K as resolveEnabled, B as shallowEqualObjects, M as resolveStaleTime, H as noop, N as environmentManager, O as isValidTimeout, Q as timeUntilStale, U as timeoutManager, W as focusManager, X as fetchState, Y as replaceData, G as notifyManager, I as shouldThrowError, a as useQueryClient } from "./router-GsH7DwrS.js";
import { r as reactExports } from "./worker-entry-CW2L5Pfm.js";
...
const api = {
  ...
  listAppointments: async () => {
    const { data, error } = await supabase.from("appointments").select("*, customer:customers(*), service:services(*)").order("start_at", { ascending: false });
    if (error) throw error;
    return data;
  },
  ...
  // Payments
  listPayments: async () => {
    const { data, error } = await supabase.from("payments").select("*, appointment:appointments(*, customer:customers(*), service:services(*))").order("created_at", { ascending: false });
    if (error) throw error;
    return data;
  },
